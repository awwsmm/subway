...
impl Authenticators {
    pub(crate) fn get_user(&self, token: Token) -> Option<User> {
        match self {
            Authenticators::Keycloak(inner) => inner.state.get_user(token),
            Authenticators::InMemory(inner) => inner.state.get_user(token),
        }
    }

    pub(crate) async fn login(&mut self, username: String, password: String) -> Result<Token, String> {
        match self {
            Authenticators::Keycloak(inner) => inner.login(username, password).await,
            Authenticators::InMemory(inner) => inner.login(username, password).await,
        }
    }
}

pub(crate) struct KeycloakAuthenticator {
...
pub(crate) struct InMemoryAuthenticator {
    state: AuthenticatorState,
}
